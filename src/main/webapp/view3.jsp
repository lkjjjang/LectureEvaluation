...
<body>
	...
	<div class="container">
				...
				<tbody>
					...
					<tr id="upvote_bbsID">
						<td colspan="3" style="text-align: center;">
							<button onclick="upvoteUpdate(${detail.bbsID})" class="btn btn-primary">${detail.bbsUpvote} 추천</button>
						</td>
					</tr>
				</tbody>
			</table>
		...
		<div style="text-align: right;">
			...
			<button class="btn btn-outline-secondary" id="bbsContentDelete_${detail.bbsID}">삭제</button>
		</div>
	</div>
	...
	<script type="text/javascript">
		...
		function openUpdateForm(x, y, tagID) {
			var options = 'width=200, height=100, top='+ y +', left='+ x +', resizable=no, scrollbars=no, location=no'; 
			window.open('./bbsDelete.jsp?id=' + (tagID.id) + '', 'passCheck', options);
		}
		...
		function replyPrint(json, id) {
			...
			for (var i = 0; i < commentCount; i++) {
				...
				row =	'<tr>' +
							...
							'<td style="text-align: right; font-size: 12px">' +
								json[i].regDate +
								'<button class="btn btn-light" id="delID_'+ commentID +'" style="font-size: 12px">[삭제]</button>' +
							'</td>' +
						'</tr>' +
						'<!-- 대댓글 등록 -->' +
					...
					for (var j = 0; j < reply.length; j++) {
						row +=  '<!-- 대댓글 리스트 -->' +
								'<tr id="reply_table_'+ commentID +'" bgcolor="#F5F5F5">' +
									...
									'</td>' +
									'<td style="text-align: right; font-size: 12px">' +
										reply[j].regDate + '<button class="btn btn-light" id="delID_' + commentID + '" style="font-size: 12px">[삭제]</button>' +
									'</td>' +
								'</tr>';
					}	
				}		
			...
			var commentTest = document.getElementById('commentTest') + commentCount;
	...
	<script type="text/javascript">
		...
		function commentsPrint(json) {			
			...
			for (var i = 0; i < commentCount; i++) {
				var commentID = json[i].commentID;
				var writeID = json[i].writeID;
				console.log('comment : ' + json[i].comment);
				row =	'<tr>' +
							'<td style="text-align: left;">' +
								writeID +
							'</td>' +
